    ...
    public function enqueue_scripts_styles() {
        if (is_singular('sfwd-quiz')) {
            ...
            if ($has_sidebar === '1' || $has_sidebar === 'yes' || $has_sidebar === true) {
                ...
                // Enqueue optimized performance script for large quizzes
                $optimized_js_path = LILAC_QUIZ_SIDEBAR_PLUGIN_DIR . 'assets/js/optimized-quiz-performance.js';
                $optimized_js_url = LILAC_QUIZ_SIDEBAR_PLUGIN_URL . 'assets/js/optimized-quiz-performance.js';
                $optimized_js_ver = file_exists($optimized_js_path) ? filemtime($optimized_js_path) : LILAC_QUIZ_SIDEBAR_VERSION;
                
                wp_enqueue_script(
                    'lilac-quiz-optimized-performance',
                    $optimized_js_url,
                    array('jquery'),
                    $optimized_js_ver,
                    true
                ...
                // Skip old scripts - optimized script handles everything
                // wp_enqueue_script('lilac-quiz-sidebar-media', $js_url, array('jquery'), $js_ver, true);
                
                // Skip enhanced layout - optimized script handles this too
                // $enhanced_js_path = LILAC_QUIZ_SIDEBAR_PLUGIN_DIR . 'assets/js/enhanced-sidebar-layout.js';
                
                // Localize the script with quiz data for optimized script
                wp_localize_script('lilac-quiz-optimized-performance', 'lilacQuizSidebar', array(
            ...
            // Skip hint enforcement scripts - optimized script handles this
            if ($enforce_hint === '1') {
                // Only load CSS for styling
                wp_enqueue_style(
                ...
                // Skip all JavaScript - optimized script handles hint enforcement
            }
        }
    }
